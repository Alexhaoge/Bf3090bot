...
async def upd_gameId():
    ...
    gameIdList = []
    tasks = []
    remid, sid, sessionID = db_op("SELECT remid, sid, sessionid FROM bf1admins ORDER BY RANDOM() LIMIT 1;", [])[0]
    for _ in range(30):
    ...
    for game_id in gameIdList:
        ...
        if len(tasks) == 250:
            print(f"开始获取私服详细信息，共{len(tasks)}个")
    ...
    pids = [r[0] for r in db_op('SELECT pid FROM bf1admins')]
    server_bf1admins = {}

    for serverid, adlist in admin_dict.items():
        pid = set(int(ad["personaId"]) for ad in adlist).intersection(pids)
        if pid:
            server_bf1admins[int(serverid)] = pid.pop()
    db_op_many('UPDATE serverbf1admins SET pid=? WHERE serverid=?;', server_bf1admins.items())
    exist_admin_serverids = set(db_op('SELECT serverid FROM serverbf1admins', []))
    new_admin_serverids = list(set(server_bf1admins.keys()).difference(exist_admin_serverids))
    db_op_many('INSERT serverbf1admins (serverid, pid) VALUES (?, ?);',
               [(sid, server_bf1admins[sid]) for sid in new_admin_serverids])
...
def renew():
    ...
    gsbinds = db_op('SELECT groupqq, ind, serverid FROM groupservers;', [])
    ...
    for groupqq, server_ind, serverid in gsbinds:
        ...
        redis_client.set(f'gameid:{serverid}', int(info[str(serverid)]["server_game_id"]))
